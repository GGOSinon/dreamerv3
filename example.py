def main():
  ...
  logger = embodied.Logger(step, [
      ...
      embodied.logger.WandBOutput(r'.*', env_name, config, ''),
  ...
  env = gym.make(env_name); dataset = env.get_dataset()
  ...
  for i in range(N):
      ...
      replay.add({'id': 0,
                  'vector': dataset['observations'][i],
                  'action': dataset['actions'][i],
                  'reward': dataset['rewards'][i],
                  'is_first': is_first,
                  'is_last': is_last,
                  'is_terminal': dataset['terminals'][i],})
      is_first = is_last
